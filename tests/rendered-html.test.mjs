...
test("server-renders the AgentBait research feature", async () => {
  ...
  assert.match(html, /Policy πθ proposes strategy s/);
  assert.match(html, /xB \+ s → x′B/);
  assert.match(html, /Target · Selected/);
  assert.match(html, /Overview of our advisor–rewriter setting\./);
  ...
  assert.match(settingText, /01 · Read Trained Advisor/);
