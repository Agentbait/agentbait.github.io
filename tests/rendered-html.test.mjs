...
test("ships the manuscript and method figure", async () => {
  ...
  assert.match(pageSource, /className="quill-ink-contact"/);
  ...
  assert.match(globalStyles, /\.rewrite-hand-motion\s*\{[^}]*left:\s*30%[^}]*top:\s*4px[^}]*transform-origin:\s*43px 248px[^}]*animation:\s*rewriter-hand-left/s);
  assert.match(globalStyles, /@keyframes rewriter-hand-left\s*\{[\s\S]*?57%,60%[^}]*translate\(110px, -150px\)[\s\S]*?63%[^}]*translate\(-110px, -150px\)[\s\S]*?67\.4%[^}]*translate\(110px, -131px\)[\s\S]*?70\.5%[^}]*translate\(110px, -58px\)[\s\S]*?78%[^}]*translate\(-115px, -24px\)/);
  ...
  assert.match(pageSource, /ink-write-title-one[\s\S]*What Makes a Model[\s\S]*ink-write-title-two[\s\S]*Choose This\?/);
  assert.match(pageSource, /ink-write-abstract-one[\s\S]*A controlled rewrite reveals[\s\S]*ink-write-abstract-two[\s\S]*which presentation cues redirect[\s\S]*ink-write-abstract-three[\s\S]*the same chooser/);
  ...
  assert.match(globalStyles, /\.ink-write-line\s*\{[^}]*clip-path:\s*inset\(0 0 0 100%\)/s);
  ...
  assert.match(globalStyles, /@keyframes rewriter-write-title-one/);
  assert.match(globalStyles, /@keyframes rewriter-write-title-two/);
  assert.match(globalStyles, /@keyframes rewriter-write-abstract-one/);
  assert.match(globalStyles, /@keyframes rewriter-write-abstract-two/);
  assert.match(globalStyles, /@keyframes rewriter-write-abstract-three/);
  assert.match(globalStyles, /@keyframes rewriter-ink-contact/);
  assert.match(globalStyles, /@media \(prefers-reduced-motion: reduce\)[\s\S]*?\.ink-write-line\s*\{[^}]*opacity:\s*1[^}]*clip-path:\s*inset\(0\)/s);
