...
test("ships the manuscript and method figure", async () => {
  ...
  assert.match(pageSource, /className="rewrite-output-flight"[\s\S]*Rewrite complete[\s\S]*Candidate B/);
  assert.doesNotMatch(pageSource, /className="rewrite-transfer"|className="cross-panel-path/);
  assert.match(globalStyles, /\.rewrite-output-flight\s*\{[^}]*left:\s*35%[^}]*top:\s*79%[^}]*animation:\s*rewrite-b-reinsert var\(--system-cycle\)/s);
  assert.match(globalStyles, /@keyframes rewrite-b-reinsert\s*\{[\s\S]*?left:\s*68\.8%;\s*top:\s*53%;[\s\S]*?min-height:\s*88px/);
  ...
  assert.match(globalStyles, /\.selected-candidate em\s*\{[^}]*animation:\s*selection-label-reveal var\(--system-cycle\)/s);
  assert.match(globalStyles, /@keyframes selection-label-reveal\s*\{[\s\S]*?0%,85%[\s\S]*?opacity:\s*0[\s\S]*?87%,100%[\s\S]*?opacity:\s*1/);
  ...
  assert.match(globalStyles, /@keyframes advisor-slate-arrive\s*\{[\s\S]*?24%,96%\s*\{\s*opacity:\s*\.42;\s*transform:\s*translate\(0, 0\);\s*\}[\s\S]*?99%,100%\s*\{\s*opacity:\s*0;/);
  assert.doesNotMatch(globalStyles, /@keyframes advisor-slate-arrive[^}]*translate\(8%, 0\)/s);
  assert.match(globalStyles, /@keyframes advisor-nontarget-recede\s*\{[\s\S]*?21%,96%\s*\{\s*opacity:\s*\.72;\s*\}/);
  assert.doesNotMatch(globalStyles, /@keyframes advisor-nontarget-recede[^}]*translateX/s);
  assert.match(globalStyles, /@keyframes advisor-source-target\s*\{[\s\S]*?23%,96%\s*\{\s*opacity:\s*\.58;[\s\S]*?box-shadow:\s*inset 3px 0 0 rgba\(188,73,63,\.45\);[\s\S]*?transform:\s*translateX\(0\);/);
