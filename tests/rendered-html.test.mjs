...
test("server-renders the AgentBait research feature", async () => {
  ...
  assert.match(finding3, /Selection success and factual quality can diverge/);
...
test("ships the manuscript and method figure", async () => {
  ...
  assert.match(globalStyles, /@keyframes advisor-reward-glow/);
  assert.match(globalStyles, /99%\s*\{\s*filter:\s*drop-shadow\(0 0 8px rgba\(188,73,63,\.22\)\)/);
