...
test("ships the manuscript and method figure", async () => {
  ...
  assert.match(pageSource, /className="advisor-intake-animation" aria-hidden="true"[\s\S]*className="upstream-slate"[\s\S]*className="upstream-target"[\s\S]*className="target-extraction-trace"/);
  ...
  assert.match(globalStyles, /\.target-extraction-trace\s*\{[^}]*transform:\s*rotate\(32deg\) scaleX\(0\)[^}]*animation:\s*advisor-extraction-trace/s);
  ...
  const advisorSourceTargetFrames = sliceBetween(globalStyles, "@keyframes advisor-source-target", "@keyframes advisor-extraction-trace");
  ...
  assert.match(globalStyles, /@keyframes advisor-extraction-trace/);
  assert.doesNotMatch(globalStyles, /@keyframes advisor-target-extract/);
