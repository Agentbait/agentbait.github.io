...
test("ships the manuscript and method figure", async () => {
  ...
  assert.match(globalStyles, /@keyframes strategy-token-delivery/);
  assert.match(pageSource, /className="cross-panel-path strategy-path"[\s\S]*className="strategy-source-port"[\s\S]*className="strategy-token">Strategy s<\/span>/);
  ...
  assert.match(globalStyles, /\.strategy-path::before\s*\{[^}]*content:\s*"controls rewrite"/s);
  assert.match(globalStyles, /\.strategy-path \.strategy-source-port\s*\{[^}]*border:\s*2px solid var\(--red\)/s);
  assert.match(globalStyles, /\.strategy-token\s*\{[^}]*background:\s*var\(--red\)[^}]*animation:\s*strategy-token-delivery 9s/s);
  ...
  assert.match(globalStyles, /\.rewriter-control-input\s*\{[^}]*left:\s*53%[^}]*top:\s*16px[^}]*animation:\s*rewriter-control-dock 9s/s);
  assert.match(globalStyles, /\.rewriter-control-input::before\s*\{[^}]*border:\s*2px solid var\(--red\)/s);
  assert.match(globalStyles, /\.rewriter-control-input::after\s*\{[^}]*height:\s*76px[^}]*animation:\s*rewriter-control-tether 9s/s);
  assert.match(globalStyles, /@keyframes strategy-token-delivery\s*\{[\s\S]*?left:\s*calc\(100% - 60px\); opacity:\s*1;[\s\S]*?\}/);
