...
test("server-renders the AgentBait research feature", async () => {
  ...
  assert.doesNotMatch(html, /Outside advisor · fixed slate/);
...
test("ships the manuscript and method figure", async () => {
  ...
  assert.match(pageSource, /<small>Control input<\/small><b>Strategy s<\/b>/);
  ...
  assert.match(globalStyles, /\.advisor-target-document\s*\{[^}]*opacity:\s*0[^}]*transform:\s*translateY\(18px\) scale\(\.98\)[^}]*animation:\s*advisor-target-receive/s);
  assert.match(globalStyles, /@keyframes advisor-target-receive\s*\{[\s\S]*?0%,8%[\s\S]*?13%,24%[\s\S]*?28%,100%/);
  assert.doesNotMatch(pageSource, /advisor-intake-animation|upstream-slate|target-extraction-trace|advisor-input-boundary/);
  assert.doesNotMatch(globalStyles, /\.upstream-slate|@keyframes advisor-slate-arrive|@keyframes advisor-target-extract/);
  ...
  assert.match(globalStyles, /@keyframes rewriter-hand-left\s*\{[\s\S]*?0%,57%,100%[\s\S]*?60%[\s\S]*?68%,70%[\s\S]*?73%[\s\S]*?\}/);
  ...
  assert.doesNotMatch(globalStyles, /@keyframes advisor-slate-arrive|@keyframes advisor-nontarget-recede|@keyframes advisor-source-target|@keyframes advisor-target-extract/);
